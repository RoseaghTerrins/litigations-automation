...
create procedure dbo.YU_CLIENT_TRANSACTION_INSERT
as
...
SELECT		acc.Accountid
			...
			, acc.ClientReference 
			, convert(nvarchar(255),convert(date,d.transactiondate,105)) as [Date]
			...
			, case when AdjustmentTypeCode in ('pay','prv') then
													case when ass.segment ='YU_Backbook' then '30'
														 ...
														 when ass.segment = 'Litigation' then '32'
			...
			, case when AdjustmentTypeCode in ('pay','prv')  and segment in ('YU_Backbook','YU_Blended') then '5'
					when AdjustmentTypeCode in ('pay','prv')  and segment in ('Litigation') then '3'
...
join		(
			...
			) d on d.ClientReference= acc.ClientReference
